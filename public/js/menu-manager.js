...
class MenuManager {
    ...
    updateButtonStates() {
        ...
        this.updateCaptionsButtonState();
    }

    /** 字幕ボタンの ON(active)/OFF 表示を settings.captionsEnabled に合わせる */
    updateCaptionsButtonState() {
        if (!this.captionsBtn) return;
        this.captionsBtn.classList.toggle('active', !!this.settings.captionsEnabled);
    }

    /**
     * 字幕表示 ON/OFF を切り替え、設定を永続化して CaptionManager へ反映。
     */
    toggleCaptions() {
        ...
        if (this.captionManager) {
            this.captionManager.setEnabled(this.settings.captionsEnabled);
        }
    }

    /**
     * CaptionManager を登録し、現在の設定値で初期化する（main.js から呼ぶ）。
     * @param {object} captionManager
     */
    setCaptionManager(captionManager) {
