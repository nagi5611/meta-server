...
- **送信**: `report-ping` に `pingMs` に加え `fpsSample`（可視/XR 時のみ 10 秒ごと 1 秒窓のフレーム数）、`perfTier`（`low`≤25 / `medium`≤45 / `high`）、`loafCount`・`longtaskCount`（前回 ping からの差分）、任意 `perfSampleAt`。
