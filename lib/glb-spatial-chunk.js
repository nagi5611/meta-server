...
import { getGltfTransformIO, setGlbSpatialChunkProcessing } from './glb-texture-resize.js';
import { CHUNK_MANIFEST_SUFFIX } from './chunk-manifest-constants.js';
...
export async function runGlbSpatialChunkIfNeeded(buffer, options) {
    ...
    setGlbSpatialChunkProcessing(true);
    try {
        const io = await getGltfTransformIO();
        ...
        const manifestName = `${basename}${CHUNK_MANIFEST_SUFFIX}`;
        ...
        setGlbSpatialChunkProcessing(false);
    }
}
