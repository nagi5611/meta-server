...
import {
    ...
    isS3ModelsSyncPruneEnabled,
} from '../config/s3-assets.js';

/** S3 PutObject 既定。ファイル名が `_v_<token>` で一意である前提で immutable を許容する */
export const DEFAULT_ASSET_CACHE_CONTROL = 'public, max-age=31536000, immutable';

/** 同期マニフェスト・active.json 等メタデータ JSON は頻繁更新されるため短期キャッシュ */
 ...
 * @param {string} [cacheControlHint] 省略時は DEFAULT_ASSET_CACHE_CONTROL
 * @param {{ s3RelativePrefix?: string }} [uploadOpts] S3 キーにのみ付与（ローカル root からの相対の前に付く。例: `plane` → `plane/foo.glb`）
 * @returns {Promise<string>} S3 オブジェクトキー
 */
export async function uploadLocalModelsFile(localAbsPath, modelsDirAbs, contentTypeHint, cacheControlHint, uploadOpts = {}) {
    ...
    const pfx = String(uploadOpts.s3RelativePrefix || '').replace(/^\/+|\/+$/g, '');
    if (pfx) {
        posixRel = posixRel ? `${pfx}/${posixRel}` : pfx;
    }
 ...
 * @param {string} modelsDirAbs
 * @param {{ s3RelativePrefix?: string }} [opts]
 */
export async function uploadLocalModelsPathsOrRollbackS3(localAbsPaths, modelsDirAbs, opts = {}) {
    ...
    try {
        for (let i = 0; i < localAbsPaths.length; i += concurrency) {
            ...
            const settled = await Promise.allSettled(
                batch.map((abs) => uploadLocalModelsFile(abs, modelsDirAbs, undefined, undefined, opts)),
            );
            /** @type {string[]} */
            const batchKeys = [];
            /** @type {unknown} */
 ...
 * @returns {object}
 */
function buildManifestPayload(diskState, remoteFiles = null) {
    /** @type {Record<string, { mtimeMs: number, name: string }>} */
    const files = {};
        ...
        for (const [p, meta] of Object.entries(remoteFiles)) {
            if (meta && typeof meta === 'object') {
                files[p] = {
                    ...
                    name: typeof meta.name === 'string' ? meta.name : path.posix.basename(p),
                };
            }
        }
    }
...
/**
 * マニフェスト同期でアップロードすべきローカルパスを決める（mtime 不一致、または mtime 一致でも S3 に無い）
 * @param {Record<string, { mtimeMs: number, name: string }>} diskState
 * @param {Record<string, { mtimeMs?: number }>} remoteFiles
 * @param {(msg: string) => void} log
 * @param {number} concurrency
 * @returns {Promise<string[]>}
 */
async function resolveManifestSyncUploadList(diskState, remoteFiles, log, concurrency) {
    /** @type {string[]} */
    const mtimeMismatch = [];
    /** @type {string[]} */
    const mtimeMatched = [];
    for (const [p, meta] of Object.entries(diskState)) {
        ...
        if (!rm || Number(rm.mtimeMs) !== meta.mtimeMs) {
            mtimeMismatch.push(p);
        } else {
            mtimeMatched.push(p);
        }
    }
    ...
    /** @type {string[]} */
    const missingOnS3 = [];
    for (let i = 0; i < mtimeMatched.length; i += concurrency) {
        const batch = mtimeMatched.slice(i, i + concurrency);
        const settled = await Promise.allSettled(
            batch.map(async (posixRel) => {
                const key = s3KeyForModelsRelative(posixRel);
                const etag = await headEtag(key);
                if (!etag) {
                    missingOnS3.push(posixRel);
                    log(`upload needed (missing on S3) ${key}`);
                }
            }),
        );
        for (const s of settled) {
            if (s.status === 'rejected') {
                console.error('[s3-sync] HeadObject failed:', s.reason);
            }
        }
    }
    return [...mtimeMismatch, ...missingOnS3];
}
...
async function syncStartupLegacyMd5AndHead(modelsDirAbs, opts = {}) {
    ...
    const prune = isS3ModelsSyncPruneEnabled();
    ...
    } else if (errors === 0 && !prune) {
        log('orphan cleanup skipped (META_MODELS_S3_SYNC_PRUNE=0, shared bucket)');
    }
    return { uploaded, skipped, deleted: deletedOrphans, errors };
}

/**
 * 起動時: マニフェスト比較で S3 とローカル models を同期（欠損・mtime 不一致・S3 実体欠落時は再 Put・マニフェスト上の孤立パスを削除）。
...
export async function syncLocalModelsToS3OnStartup(modelsDirAbs, opts = {}) {
    ...
    const prune = isS3ModelsSyncPruneEnabled();
    if (!prune) {
        log('META_MODELS_S3_SYNC_PRUNE=0: will upload local changes only; no startup S3 deletes');
    }
    ...
    if (remoteFiles === null) {
        ...
        if (errors === 0) {
            try {
                ...
                const remoteForManifest =
                    !prune && remoteParsed?.files && typeof remoteParsed.files === 'object'
                        ? /** @type {Record<string, { mtimeMs?: number, name?: string }>} */ (remoteParsed.files)
                        : null;
                const payload = buildManifestPayload(freshDisk, remoteForManifest);
    ...
    const toUpload = await resolveManifestSyncUploadList(diskState, remoteFiles, log, concurrency);
    /** @type {string[]} */
    const toDelete = [];
    if (prune) {
    ...
    if (toUpload.length === 0 && toDelete.length === 0) {
        ...
        const payload = buildManifestPayload(diskState, prune ? null : remoteFiles);
    ...
    if (errors === 0) {
        try {
            const freshDisk = collectLocalDiskModelStateForSync(modelsDirAbs);
            const remoteForManifest = prune ? null : remoteFiles;
            await persistSyncManifest(modelsDirAbs, buildManifestPayload(freshDisk, remoteForManifest));
        } catch (e) {
            errors++;
...
/**
 * 起動時: plane/ 配下を S3 へ（キーは models プレフィックス配下の plane/...。マニフェスト同期対象外のため Head/MD5 で欠損 Put）
 * @param {string} planeDirAbs
 * @param {{ onLog?: (msg: string) => void }} [opts]
 * @returns {Promise<{ uploaded: number, skipped: number, errors: number, mode: 'legacy' | 'noop' }>}
 */
export async function syncLocalPlaneToS3OnStartup(planeDirAbs, opts = {}) {
    const log = opts.onLog || ((m) => console.log(`[s3-sync-plane] ${m}`));
    if (!isS3ModelsBucketConfigured() || !fs.existsSync(planeDirAbs)) {
        return { uploaded: 0, skipped: 0, errors: 0, mode: 'noop' };
    }

    const disk = collectLocalDiskModelStateForSync(planeDirAbs);
    ...
    for (const posixRel of Object.keys(disk)) {
        const absFile = path.join(planeDirAbs, ...posixRel.split('/'));
        const key = s3KeyForModelsRelative(`plane/${posixRel}`);
        try {
            const localMd5 = await md5FileHex(absFile);
            const remote = await headEtag(key);
            if (remote && !remote.includes('-') && remote.length === 32 && remote === localMd5) {
            ...
            await uploadLocalModelsFile(absFile, planeDirAbs, undefined, undefined, { s3RelativePrefix: 'plane' });
            uploaded++;
            log(`uploaded ${key}`);
        } catch (e) {
            errors++;
            console.error(`[s3-sync-plane] failed ${key}:`, e);
        }
    }
