...
import { envModelManifestComputeHash } from './model-manifest.js';
 ...
 * @param {{ maxEdgePx?: number, skipTextureResize?: boolean, prefabBaseOverride?: string }} glbOptions
...
export async function applyPrefabBundleZipToModels(zipBuffer, modelsDir, zipOriginalName, confirm, glbOptions = {}, hooks = {}) {
    ...
    try {
        ...
        /** @type {{ file: string, version?: string, contentHash?: string | null, size?: number, updatedAt?: string }[]} */
        const parts = [];
        for (const e of glbEntries) {
            ...
            /** @type {string | null} */
            let partHash = null;
            if (envModelManifestComputeHash()) {
                partHash = crypto.createHash('sha256').update(fs.readFileSync(outAbs)).digest('hex');
            }
            parts.push({
                file: outRel,
                version: `${Math.floor(st.mtimeMs)}_${st.size}`,
        ...
        return {
            success: true,
            manifestRelativePath: 'models/' + manifestDestRel.split(path.sep).join('/'),
            prefabGroupId,
            displayName: base,
