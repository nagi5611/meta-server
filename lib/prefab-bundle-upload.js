...
import { runGlbPrefabProcessQueued, parseTextureMaxEdgeFromUploadBody } from './glb-texture-resize.js';
import { mergeGlbBoundsList, roundGlbBounds } from './glb-bounds.js';
...
export async function applyPrefabBundleZipToModels(zipBuffer, modelsDir, zipOriginalName, confirm, glbOptions = {}, hooks = {}) {
    ...
    const written = [];
    /** @type {Map<string, import('./glb-bounds.js').GlbBounds | null>} */
    const glbBoundsByRel = new Map();
    try {
        for (const e of entries) {
            ...
            if (e.ext === '.glb') {
                ...
                const r = await runGlbPrefabProcessQueued(buf, { maxEdgePx, skipTextureResize });
                const toWrite = r.buffer;
                glbBoundsByRel.set(e.rel, r.bounds);
        ...
        const partBoundsList = [];
        const parts = glbEntries.map((e) => {
            const file = `${publicUrlPrefix}/${path.join(assetDirRel, e.rel).split(path.sep).join('/')}`.replace(/\/+/g, '/');
            const rawBounds = glbBoundsByRel.get(e.rel);
            const part = { file };
            if (rawBounds) {
                const b = roundGlbBounds(rawBounds);
                part.bounds = b;
                partBoundsList.push(b);
            }
            return part;
        });

        const mergedBounds = mergeGlbBoundsList(partBoundsList);
        /** @type {Record<string, unknown>} */
        const manifest = {
            version: mergedBounds ? 2 : 1,
            prefabGroupId,
            displayName: base,
            parts,
        };
        if (mergedBounds) {
            manifest.bounds = roundGlbBounds(mergedBounds);
        }
