...
const PREFAB_ZIP_MAX_ENTRIES = 500;
 ...
 * @param {{ maxEdgePx?: number, skipTextureResize?: boolean }} glbOptions
...
export async function applyPrefabBundleZipToModels(zipBuffer, modelsDir, zipOriginalName, confirm, glbOptions = {}, hooks = {}) {
    ...
    const base = baseNameFromZipFilename(zipOriginalName);
    ...
    try {
        ...
        const parts = glbEntries.map((e) => ({
            file: path.join(assetDirRel, e.rel).split(path.sep).join('/'),
        }));
