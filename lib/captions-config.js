...
/**
 * 字幕翻訳の明示的オプトアウト判定。
 * ENABLE_CAPTION_TRANSLATION が未設定のときは false（= 既定 ON）。
 * @returns {boolean}
 */
function isCaptionTranslationExplicitlyDisabled() {
    const raw = process.env.ENABLE_CAPTION_TRANSLATION;
...
/**
 * 値がファイルパスっぽいか（JSON 本文ではない）
 * @param {string} value
 * @returns {boolean}
 */
function looksLikeCredentialsFilePath(value) {
...
export function resolveSpeechCredentials() {
    ...
    if (inlineJson) {
        // よくある誤設定: パスを GOOGLE_SPEECH_CREDENTIALS_JSON に入れる
        if (looksLikeCredentialsFilePath(inlineJson)) {
            console.warn(
                '[captions] GOOGLE_SPEECH_CREDENTIALS_JSON にファイルパスが入っています。',
                'パス指定は GOOGLE_APPLICATION_CREDENTIALS を使ってください（自動で読み替えます）。',
            );
            process.env.GOOGLE_APPLICATION_CREDENTIALS = inlineJson;
            return { mode: 'adc' };
        }
        ...
        try {
            ...
            console.error(
                '[captions] GOOGLE_SPEECH_CREDENTIALS_JSON is not valid JSON:',
                e.message,
                '— ファイルパスの場合は GOOGLE_APPLICATION_CREDENTIALS=/path/to/key.json を使ってください。',
                'インライン JSON の場合は {"type":"service_account",...} 形式の全文を設定してください。',
            );
            return null;
        }
    }
...
/**
 * 字幕 final 時の Google Translation API が利用可能か。
 * 既定: 字幕が有効かつ GCP 資格情報がある場合は ON。
 * @returns {boolean}
 */
export function isCaptionTranslationReady() {
    if (isCaptionTranslationExplicitlyDisabled()) return false;
    if (!CAPTIONS_ENABLED) return false;
    return resolveSpeechCredentials() != null;
}
