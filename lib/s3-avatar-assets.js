 ...
 */
/**
 * ローカルに存在するアバター GLB を S3 に揃える（署名 URL 発行前に呼ぶ）
 * @param {string} avatarsDirAbs
 * @param {string} glbFilename avatars 直下のファイル名
 * @returns {Promise<boolean>} ローカルに無ければ false、それ以外は Put 成功時 true
 */
export async function ensureAvatarGlbOnS3(avatarsDirAbs, glbFilename) {
    const fn = path.basename(String(glbFilename || '').replace(/[/\\]/g, ''));
    if (!fn.toLowerCase().endsWith('.glb')) return false;
    const abs = path.join(avatarsDirAbs, fn);
    if (!fs.existsSync(abs)) return false;
    if (!isS3ModelsBucketConfigured()) return true;
    const key = s3KeyForAvatarRelative(fn);
    try {
        const localMd5 = await md5FileHex(abs);
        const remote = await headEtag(key);
        if (remote && !remote.includes('-') && remote.length === 32 && remote === localMd5) {
            return true;
        }
        await uploadLocalAvatarFile(abs, avatarsDirAbs);
        return true;
    } catch (e) {
        console.error(`[s3-avatar] ensureAvatarGlbOnS3 failed ${key}:`, e);
        return false;
    }
}

/**
 * CDN 論理パス（例 diorama/avatars/foo.glb）に対応する同一オリジン配信パス
 * @param {string} pathOrFilename
 * @returns {string}
 */
export function originAvatarPathForLogicalAsset(pathOrFilename) {
    const posix = String(pathOrFilename || '').replace(/^\/+/, '').replace(/\\/g, '/');
    const m = posix.match(/^(?:[^/]+\/)?avatars\/(.+)$/i);
    const tail = m ? m[1] : path.basename(posix);
    if (!tail) return '/avatars/';
    return `/avatars/${tail.split('/').map((s) => encodeURIComponent(s)).join('/')}`;
}
