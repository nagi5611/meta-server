...
export const AIRCRAFT_PHYSICS_LIMITS = {
    ...
    throttleSpoolPerS: [0.05, 5],
    maxBankDeg: [1, 85],
    rudderAuthorityRefSpeedMs: [0, 500],
    rudderAuthorityMinScale: [0.05, 1],
    thrustPitchFromThrottleDelta: [0, 2],
    thrustPitchRelaxNoInput: [0, 20],
    flapPitchDownAuthority: [0, 1],
};
...
export function validateWorldsAircraft(worlds) {
    ...
    for (const [wid, w] of Object.entries(worlds)) {
        ...
        models.forEach((m, i) => {
            ...
            const be = /** @type {Record<string, unknown>} */ (a).bodyEulerDeg;
            if (be != null) {
                if (typeof be !== 'object' || Array.isArray(be)) {
                    errors.push(`ワールド「${wid}」 aircraft「${id}」: bodyEulerDeg は { x, y, z } オブジェクトにしてください`);
                } else {
                    for (const k of ['x', 'y', 'z']) {
                        const v = be[k];
                        if (v != null && (typeof v !== 'number' || !Number.isFinite(v))) {
                            errors.push(`ワールド「${wid}」 aircraft「${id}」: bodyEulerDeg.${k} は有限数値にしてください`);
                        }
                    }
                }
            }
