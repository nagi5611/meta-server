...
/** aircraftPhysics の既知キー一覧（POST /admin/worlds 用・数値は下限・上限とも検証しない） */
...
/** easy 操縦（flightPhysicsEasy）の既知キー一覧 */
...
/**
 * aircraftPhysics オブジェクトの既知キーを有限数値かどうかだけ検証する（上下限なし）
 * @param {unknown} ap
 * @param {string} pathLabel 例: ワールド「lobby」: aircraftPhysics
 * @param {string[]} errors
 * @param {Record<string, number>} [knownKeysTable] キー一覧（値は未使用）
 */
export function appendAircraftPhysicsValidationErrors(ap, pathLabel, errors, knownKeysTable = AIRCRAFT_PHYSICS_MIN) {
    if (ap == null) return;
    if (typeof ap !== 'object' || Array.isArray(ap)) {
        errors.push(`${pathLabel} はオブジェクトである必要があります`);
        return;
    }
    for (const key of Object.keys(knownKeysTable)) {
