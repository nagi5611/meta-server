...
export function normalizeGeoConfig(raw) {
    ...
    const overlayLat = finiteNum(src.overlayCenterLat);
    const overlayLng = finiteNum(src.overlayCenterLng);
    const overlayZoomRaw = finiteNum(src.overlayZoom);
    const overlayHeadingRaw = finiteNum(src.overlayHeading);
    return {
        enabled,
        anchorWorldX,
        anchorWorldZ,
        ...
        zoomOffset,
        headingMode,
        ...(overlayLat != null ? { overlayCenterLat: Math.max(-90, Math.min(90, overlayLat)) } : {}),
        ...(overlayLng != null ? { overlayCenterLng: Math.max(-180, Math.min(180, overlayLng)) } : {}),
        ...(overlayZoomRaw != null && overlayZoomRaw >= 1 && overlayZoomRaw <= 22
            ? { overlayZoom: Math.round(overlayZoomRaw) }
            : {}),
        ...(overlayHeadingRaw != null
            ? { overlayHeading: Math.max(-180, Math.min(180, overlayHeadingRaw)) }
            : {}),
