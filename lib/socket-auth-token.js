...
/**
 * ログイン成功時、または CDN アセット用ゲストセッションで付与する Socket 用トークンを生成する
 * @param {{ role: 'student' | 'teacher' | 'guest', ttlMs?: number }} opts
 * @returns {string}
 */
export function signSocketAuthToken({ role, ttlMs } = {}) {
    if (role !== 'student' && role !== 'teacher' && role !== 'guest') {
 ...
 * @param {unknown} token
 * @returns {{ role: 'student' | 'teacher' | 'guest' } | null}
 */
export function verifySocketAuthToken(token) {
    ...
    if (role !== 'student' && role !== 'teacher' && role !== 'guest') return null;
    return { role };
}
