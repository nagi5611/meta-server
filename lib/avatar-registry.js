...
import { isS3ModelsBucketConfigured } from '../config/s3-assets.js';
import { readActiveAvatarMeta, setActiveAvatar, uploadLocalAvatarFile } from './s3-avatar-assets.js';
...
/**
 * avatars-registry.json を S3 に Put する（短期 Cache-Control）
 * @param {string} avatarsDir
 * @returns {Promise<void>}
 */
export async function syncAvatarRegistryToS3(avatarsDir) {
    if (!isS3ModelsBucketConfigured()) return;
    const abs = registryAbsPath(avatarsDir);
    if (!fs.existsSync(abs)) return;
    await uploadLocalAvatarFile(abs, avatarsDir, MANIFEST_CACHE_CONTROL);
}

/**
 * レジストリをローカルに書き、S3 連携時は同一内容を Put する
 * @param {string} avatarsDir
 * @param {object} registry
 * @returns {Promise<void>}
 */
export async function writeAvatarRegistryAndSync(avatarsDir, registry) {
    writeAvatarRegistry(avatarsDir, registry);
    await syncAvatarRegistryToS3(avatarsDir);
}
...
export async function ensureAvatarRegistry(avatarsDir) {
    ...
    if (!reg || !Array.isArray(reg.avatars)) {
        ...
        await writeAvatarRegistryAndSync(avatarsDir, reg);
    }
    let changed = false;
