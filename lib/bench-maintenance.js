...
const BENCH_MAINTENANCE_STATUS_MESSAGE =
    '現在ベンチマーク実行中です。接続は維持されますが、性能に影響が出る場合があります。';

/**
 * @returns {{ active: boolean, runId: string | null, message: string | null }}
 */
export function getBenchMaintenancePublicStatus() {
    return {
        active: benchActive,
        runId: activeRunId,
        message: benchActive ? BENCH_MAINTENANCE_STATUS_MESSAGE : null,
    };
}
 ...
 * @returns {boolean}
 */
function shouldReceiveBenchMaintenanceStatus(socket) {
    return !socket.data?.isBenchBot;
}

/**
 * 接続中クライアントへベンチ状態を push（info パネル表示用）
 * @param {import('socket.io').Server} io
 */
export function broadcastBenchMaintenanceStatus(io) {
    if (!io) return;
    const payload = getBenchMaintenancePublicStatus();
    for (const socket of io.sockets.sockets.values()) {
        if (!shouldReceiveBenchMaintenanceStatus(socket)) continue;
        socket.emit('bench-maintenance-status', payload);
    }
}

/**
 * 新規接続クライアントへ現在のベンチ状態を送る
 * @param {import('socket.io').Socket} socket
 */
export function emitBenchMaintenanceStatusToSocket(socket) {
    if (!socket || !shouldReceiveBenchMaintenanceStatus(socket)) return;
    socket.emit('bench-maintenance-status', getBenchMaintenancePublicStatus());
}
...
export function setBenchMaintenance(opts) {
    ...
    if (io) {
        broadcastBenchMaintenanceStatus(io);
        if (benchActive && !wasActive) {
            for (const socket of io.sockets.sockets.values()) {
                if (!shouldReceiveBenchMaintenanceStatus(socket)) continue;
                socket.emit('bench-maintenance-warning', {
                    message: BENCH_MAINTENANCE_STATUS_MESSAGE,
                });
            }
        }
    }
