...
class GoogleCaptionSession {
    ...
    _openStream() {
        ...
        stream.on('error', (err) => {
            // 張り直し中の end に伴う想定内エラーは握りつぶし、その他は通知
    ...
    _scheduleRestart() {
        ...
        this.restartTimer = setTimeout(() => {
            ...
            this.stream = null;
            try { if (old) old.end(); } catch (_) { /* ignore */ }
            this._openStream();
        }, RESTART_INTERVAL_MS);
    ...
    write(pcmChunk) {
        ...
        try {
            this.stream.write({ audioContent: pcmChunk });
        } catch (_) { /* ストリーム切替の隙間は無視 */ }
    }

    close() {
        this.closed = true;
        if (this.restartTimer) { clearTimeout(this.restartTimer); this.restartTimer = null; }
        try { if (this.stream) this.stream.end(); } catch (_) { /* ignore */ }
        this.stream = null;
    }
}
