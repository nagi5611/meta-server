...
// 既定モデル: https://ai.google.dev/gemini-api/docs/models/gemini-3.1-flash-lite-preview
// GEMINI_MODEL で上書き可。一覧は https://ai.google.dev/gemini-api/docs/models を参照
...
function buildNgInjectorSection(phrasesRaw, kind) {
    ...
    return `\n\n【運営登録の禁止参照語・題材ヒント】
${scope}
次の箇条書きに**具体語として列挙されている文字列**が、評価対象を表記正規化（NFKC 等）したうえで**連続した部分文字列として含まれる**場合は、短さ・種類を問わず常に inappropriate を true にしてください（数字だけの語も含みます）。
次に列挙するものについて、(1) そのまま、またはひら／カタ／漢字・英数字などの表記ゆれ、(2) 意味がほぼ同じ言い換え・婉曲・隠語・スラング、(3) 意図が明らかな比喩や略称、が評価対象テキストに含まれる／指していると妥当に読み取れる場合も inappropriate を true にしてください。
「偶然の字面の一致」で除外してよいのは、リストに明示されていない別の無関係な語だけです。
