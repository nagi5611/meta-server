...
import {
    ...
    seedFirstRunDefaultAddons,
    getAddonEnabledMap,
    ensureWebxrVrOnUpgrade,
    ensureNfcSpawnOnUpgrade,
} from '../db/addons-registry.js';
...
/** @type {Array<{ id: string, version: string, loaded: boolean, skipReason?: string }>} */
let lastStartupReport = [];
...
export async function loadAddonsAtStartup(opts) {
    ...
    ensureWebxrVrOnUpgrade(addonsRoot);
    ensureNfcSpawnOnUpgrade(addonsRoot);
...
/**
 * 直近の起動時アドオンロード結果（管理画面診断用）
 * @returns {Array<{ id: string, version: string, loaded: boolean, skipReason?: string }>}
 */
export function getAddonStartupReport() {
...
export function getAddonCatalogSnapshot() {
    ...
    const startupById = new Map(lastStartupReport.map((row) => [row.id, row]));
    ...
    for (const dirName of dirs) {
        ...
        const startup = startupById.get(mr.manifest.id);
        addons.push({
            id: mr.manifest.id,
            ...
            enabled,
            loaded: startup?.loaded === true,
            skipReason: startup?.skipReason || null,
            coreVersion,
